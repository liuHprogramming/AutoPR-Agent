...
## CV Bullet
Built AutoPR-Agent, a multi-agent code repair system that converts bug reports into validated PR candidates using AST-based code localization, regression-test generation, patch synthesis, review, and before/after test verification; on a seeded Python benchmark suite, improved validated repair success from 0/4 for a single-agent baseline to 4/4 and improved top-1 localization from 0/4 keyword retrieval to 4/4 AST-symbol ranking.
